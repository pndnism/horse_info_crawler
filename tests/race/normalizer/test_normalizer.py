...
from tests.race.normalizer.data.test_race_detail_lists import NORMALIZED_ARRIVAL_ORDERS_LIST, NORMALIZED_BOX_NUMBERS_LIST, NORMALIZED_EARN_PRIZES_LIST, NORMALIZED_GOAL_MARGINS_LIST, NORMALIZED_GOAL_TIMES_LIST, NORMALIZED_HALF_TIMES_LIST, NORMALIZED_HORSE_AGES_LIST, NORMALIZED_HORSE_IDS_LIST, NORMALIZED_HORSE_NAMES_LIST, NORMALIZED_HORSE_NUMBERS_LIST, NORMALIZED_HORSE_OWNERS_LIST, NORMALIZED_HORSE_SEXES_LIST, NORMALIZED_HORSE_WEIGHTS_LIST, NORMALIZED_HORSE_WEIGHT_DIFFS_LIST, NORMALIZED_JOCKEY_NAMES_LIST, NORMALIZED_JOCKEY_WEIGHTS_LIST, NORMALIZED_ODDS_LIST, NORMALIZED_ORDER_TRANSITIONS_LIST, NORMALIZED_POPULARITIES_LIST, NORMALIZED_TRAINER_NAMES_LIST
...
class TestRaceDetailsNormalizer(TestCase):
    ...
    def test_normalize_box_numbers(self):
        self.assertEqual(RaceDetailsNormalizer.normalize_box_numbers(self.source_dict["枠番"]), NORMALIZED_BOX_NUMBERS_LIST)

    def test_normalize_horse_numbers(self):
        self.assertEqual(RaceDetailsNormalizer.normalize_horse_numbers(self.source_dict["馬番"]), NORMALIZED_HORSE_NUMBERS_LIST)

    def test_normalize_horse_names(self):
        self.assertEqual(RaceDetailsNormalizer.normalize_horse_names(self.source_dict["馬名"]), NORMALIZED_HORSE_NAMES_LIST)

    def test_normalize_horse_ids(self):
        self.assertEqual(RaceDetailsNormalizer.normalize_horse_ids(self.source_dict["馬名"]), NORMALIZED_HORSE_IDS_LIST)

    def test_normalize_horse_sexes(self):
        self.assertEqual(RaceDetailsNormalizer.normalize_horse_sexes(self.source_dict["性齢"]), NORMALIZED_HORSE_SEXES_LIST)

    def test_normalize_horse_ages(self):
        self.assertEqual(RaceDetailsNormalizer.normalize_horse_ages(self.source_dict["性齢"]), NORMALIZED_HORSE_AGES_LIST)

    def test_normalize_jockey_weights(self):
        self.assertEqual(RaceDetailsNormalizer.normalize_jockey_weights(self.source_dict["斤量"]), NORMALIZED_JOCKEY_WEIGHTS_LIST)

    def test_normalize_jockey_names(self):
        self.assertEqual(RaceDetailsNormalizer.normalize_jockey_names(self.source_dict["騎手"]), NORMALIZED_JOCKEY_NAMES_LIST)

    def test_normalize_goal_times(self):
        self.assertEqual(RaceDetailsNormalizer.normalize_goal_times(self.source_dict["タイム"]), NORMALIZED_GOAL_TIMES_LIST)

    def test_normalize_goal_margins(self):
        self.assertEqual(RaceDetailsNormalizer.normalize_goal_margins(self.source_dict["着差"]), NORMALIZED_GOAL_MARGINS_LIST)

    def test_normalize_order_transitions(self):
        self.assertEqual(RaceDetailsNormalizer.normalize_order_transitions(self.source_dict["通過"]), NORMALIZED_ORDER_TRANSITIONS_LIST)

    def test_normalize_half_times(self):
        self.assertEqual(RaceDetailsNormalizer.normalize_half_times(self.source_dict["上り"]), NORMALIZED_HALF_TIMES_LIST)

    def test_normalize_odds(self):
        self.assertEqual(RaceDetailsNormalizer.normalize_odds(self.source_dict["単勝"]), NORMALIZED_ODDS_LIST)

    def test_normalize_popularities(self):
        self.assertEqual(RaceDetailsNormalizer.normalize_popularities(self.source_dict["人気"]), NORMALIZED_POPULARITIES_LIST)

    def test_normalize_horse_weights(self):
        self.assertEqual(RaceDetailsNormalizer.normalize_horse_weights(self.source_dict["馬体重"]), NORMALIZED_HORSE_WEIGHTS_LIST)
    
    def test_normalize_horse_weight_diffs(self):
        self.assertEqual(RaceDetailsNormalizer.normalize_horse_weight_diffs(self.source_dict["馬体重"]), NORMALIZED_HORSE_WEIGHT_DIFFS_LIST)

    def test_normalize_trainer_names(self):
        self.assertEqual(RaceDetailsNormalizer.normalize_trainer_names(self.source_dict["調教師"]), NORMALIZED_TRAINER_NAMES_LIST)

    def test_normalize_horse_owners(self):
        self.assertEqual(RaceDetailsNormalizer.normalize_horse_owners(self.source_dict["馬主"]), NORMALIZED_HORSE_OWNERS_LIST)
    
    def test_normalize_earn_prizes(self):
        self.assertEqual(RaceDetailsNormalizer.normalize_earn_prizes(self.source_dict["賞金(万円)"]), NORMALIZED_EARN_PRIZES_LIST)